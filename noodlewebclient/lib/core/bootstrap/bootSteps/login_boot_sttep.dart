...
import 'package:noodlewebclient/core/client_helper.dart';
...
class LoginBootStep extends BootstrapStep {
  ...
  Future<bool> stepRequired(SharedPreferences prefs) async {
    ...
    if ((prefs.getBool('storeAuth') ?? false) && NoodleClientHelper.initNeeded()) {
      //2 Ways to go:
      ...
      if (kIsWeb) {
        if (document.cookie?.isEmpty ?? true) {
          ...
          if (serverUrl != null) {
            final client = NoodleClientHelper.initClient(serverUrl);
      ...
      if (serverUrl != null && (details?.present ?? false)) {
        NoodleClientHelper.initClient(serverUrl, username: details?.user, password: details?.password);
      }
    }
    //Check if client was created and has configuration for login
    return NoodleClientHelper.initNeeded();
  }
